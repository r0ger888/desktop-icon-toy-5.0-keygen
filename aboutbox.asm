...
.data
ScrollerText     db "::[The PERYFERiAH Team]:: proudly presents:..... [Desktop Icon Toy "
                 db "5.0] Keygen            Music: Hyochan, GFX+KGN: r0ger[PRF], AbtTemp: rep0A[TLG]"
                 db "                       Greetz to: Al0hA[PRF], B@TRyNU[PRF],"
                 db " GRUiA[PRF], MaryNello[PRF], WeeGee[PRF], sabYn[PRF], s0r3l[PRF], yMRAN[PRF], zzLaTaNN[PRF]"
                 db ", ShoGun[PRF], DAViD[PRF], ShTEFY[PRF], r0bica[PRF], and other [PRF] members..... als"
                 db "o: Cachito[tSRh], Talers[tSRh], Xylitol[RED], Roentgen[tSRh], kao, Arttomov[URET]..... "
                 db "and many other tsrh,exetools & trainingcircle communities.....           ",0
...
LOCAL Abtrekt:RECT
                ...
                jmp     loc_402868
loc_drawaboutbox:  
                mov     eax, 250 ; <-- form height
                mov     xHeight, eax
                mov     eax, 250 ; <-- form width
                mov     xWidth, eax                      
                invoke GetParent, hDlg
                mov ecx, eax
                invoke GetWindowRect, ecx, addr Abtrekt
                mov edi, Abtrekt.left
                mov esi, Abtrekt.top
                add edi, 0
                add esi, 0
                invoke SetWindowPos, hDlg, HWND_TOP, edi, esi, xWidth, xHeight, SWP_SHOWWINDOW
            	;invoke SetWindowPos,hDlg,0,X,Y,xHeight,xWidth,40h

                mov     edx, xWidth
                add     edx, 0Ch
           	
				
                invoke CreateBitmap,xHeight,edx,1,20h,0
                mov     BmpDC, eax           
                ...
                invoke CreateSolidBrush,0; background color                               
                mov     ho, eax
                
				
                invoke CreateFont,12,0,0,0,FW_DONTCARE,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY, 0,offset FontName;12textH 
				mov     wParam2, eax              
...
LOCAL rect:RECT
                ...
                mov     [cc], eax
                xor     edx, edx
                mov     cx, 6 ; <-- scroller length (this will restart the scroller depending on the text length)
                mul     cx
                sub     eax, 64h
                mov     [var_4], eax
...
loc_40289C:    
  ...
  ; ------------------------------------------------------------------------------------------------------------------------------------------------------
     ...
     ;-------------------------------------------------         
                ...
                push    8                  
                xor     eax, eax                
                invoke BitBlt,BoxDC,10h,xWidth,0D2h,0Ch,BoxDC,10h,0BAh,0CC0020h
                invoke GraphShake,2
                
                mov     edx,xWidth ; <-- text scroller position
                add     edx, eax
                
                invoke TextOut,BoxDC,[x],edx,offset ScrollerText,[cc]

                invoke BitBlt,BoxDC,10h,0BAh,0D2h,0Ch,BoxDC,10h,xWidth,0CC0020h
...
loc_402DB7:           
                ...
                pop     ecx
                loop    loc_402DB7
                invoke BitBlt,hDC,0,0,xHeight,xWidth,BoxDC,0,0,0CC0020h
